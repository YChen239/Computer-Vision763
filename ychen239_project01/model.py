...
def faverage(data, score):
    all = np.dot(score, data)
    k = score.shape[0]
    D = data.shape[1]
    mu = np.zeros((k,D))
    for i in range(k):
        mu[i] = all[i,:]/np.sum(score[i,:])
    return mu
...
def test(validation, k, mu,sigma, lamda, nmu, nsigma, nlamda, flag, theta=0, h=0, ntheta=0, nh=0):
    facepdf = []
    nonfacepdf = []
    if flag==0:
        for i in range(k):
    ...
    nonface = np.log(nlamda).reshape((k, 1))+nonfacepdf
...
#Model1 Single Gaussian model

u_face, sigma_face = GaussianParameters(training_face_data)
u_nonface, sigma_nonface = GaussianParameters(training_nonface_data)
...
#         np.dot((test_data[i, 0:-1] - u_face), cov1).T, (test_data[i, 0:-1] - u_face))
#     p0 = -n / 2 * np.log(2 * math.pi) - 1 / 2 * np.sum(np.log(np.square(sigma_nonface))) - 1 / 2 * np.dot(
#         np.dot((test_data[i, 0:-1] - u_nonface), cov2).T, (test_data[i, 0:-1] - u_nonface))
#     p_v = [p1, p0]
...
#
#
# parameters(predict_label, predict_label, 200)
# ROCcurve(test_data,predict_score)
#
#
#
# #Model2 Mixture of Gaussian
...
#
# k=3
# mu, sigma, lamda = EMalgorithm(face_t[0][:,:-1], k)
# nmu, nsigma, nlamda = EMalgorithm(nonface_t[0][:,:-1], 2)
# u_nonface, sigma_nonface = GaussianParameters(face_v[0][:,:-1])
# validation = np.vstack((face_v[0],nonface_v[0]))
# test(validation, 4, mu,sigma, lamda, nmu, nsigma, nlamda)

#Model3 t-distribution
# mu, sigma, lamda = EMalgorithm(training_face_data[:,:-1], 4, 1)
# nmu, nsigma, nlamda = EMalgorithm(training_nonface_data[:,:-1], 4, 1)
#
# test(test_data, 4, mu,sigma, lamda, nmu, nsigma, nlamda, 1)

#Model4
#mu, sigma, lamda, theta, h = EMalgorithm(training_face_data[:,:-1], 1, 2, 2)
# nmu, nsigma, nlamda, ntheta, nh= EMalgorithm(training_face_data[:,:-1], 1, 2, 2)
# face_u = toImage(nmu[0])
# test(test_data, 1, mu,sigma, lamda, nmu, nsigma, nlamda, 2, theta, h, ntheta, nh)
